...
def set_slice(tensor1, tensor2, h, h_len, w, w_len, mask=None):
    if mask is not None:
        tensor1[:, :, h : h + h_len, w : w + w_len] = (
            tensor1[:, :, h : h + h_len, w : w + w_len] * (1 - mask) + tensor2 * mask
        )
    else:
...
def get_tiles_and_masks_padded(steps, latent_shape, tile_height, tile_width):
    ...
    for i in range(3):
        for j in range(3):
            mask = torch.zeros(
                (batch_size, 1, tile_size_h, tile_size_w),
                dtype=torch.float32,
                device="cpu",
            )
    ...
    passes = [
        ...
        [
            [
                create_tile(h_shift, w, False, True, i, j)
                for i in range(len(h_shift))
                for j in range(len(w))
            ]
        ],
        [
            [
                create_tile(h, w_shift, True, False, i, j)
                for i in range(len(h))
                for j in range(len(w_shift))
            ]
        ],
        [
            [
                create_tile(h_shift, w_shift, False, False, i, j)
                for i in range(len(h_shift))
                for j in range(len(w_shift))
            ]
        ],
...
def mask_at_boundary(
    ...
    if (h_len == tile_size_h or h_len == latent_size_h) and (
        w_len == tile_size_w or w_len == latent_size_w
    ):
        return h, h_len, w, w_len, mask
    h_offset = min(0, latent_size_h - (h + tile_size_h))
    w_offset = min(0, latent_size_w - (w + tile_size_w))
    new_mask = torch.zeros(
        (1, 1, tile_size_h, tile_size_w), dtype=torch.float32, device=device
    )
    new_mask[
        :,
        :,
        -h_offset : h_len if h_offset == 0 else tile_size_h,
        -w_offset : w_len if w_offset == 0 else tile_size_w,
    ] = 1.0 if mask is None else mask
...
def get_tiles_and_masks_rgrid(steps, latent_shape, tile_height, tile_width, generator):
    def calc_coords(latent_size, tile_size, jitter):
        ...
        tile_coords = [
            np.clip(tile_size * c - jitter, 0, latent_size)
            for c in range(tile_coords + 1)
        ]
    ...
    for s in range(steps):
        rands = torch.rand(
            (2,), dtype=torch.float32, generator=generator, device="cpu"
        ).numpy()

        jitter_w1 = int(rands[0] * tile_size_w)
        jitter_w2 = int(((rands[0] + 0.5) % 1.0) * tile_size_w)
        jitter_h1 = int(rands[1] * tile_size_h)
        jitter_h2 = int(((rands[1] + 0.5) % 1.0) * tile_size_h)
