...
def pad_cond(tensor, repeats, empty):
    if not isinstance(tensor, dict):
        return torch.cat(
            [
                tensor,
                empty.repeat((tensor.shape[0], repeats, 1)).to(device=tensor.device),
            ],
            axis=1,
        )
        ...
        if tensor.shape[1] == uncond.shape[1] or skip_uncond:
            ...
            for batch_offset in range(0, tensor.shape[0], batch_size):
                ...
                x_out[a:b] = self.inner_model(
                    x_in[a:b],
                    sigma_in[a:b],
                    **make_condition_dict(c_crossattn, image_cond_in[a:b]),
                )
            ...
            if not skip_uncond:
                x_out[-uncond.shape[0] :] = self.inner_model(
                    ...
                    **make_condition_dict(uncond, image_cond_in[-uncond.shape[0] :]),
                )
...
def prepare_noise(latent_image, seed, noise_inds=None, device="cpu"):
    ...
    if (
        model is not None
        and (opts := model.model_options.get("smZ_opts", None)) is None
    ):
        import comfy.sample
...
def get_adm(c):
    for y in ["adm_encoded", "c_adm", "y"]:
        if y in c:
            ...
            if y == "adm_encoded":
                y = "c_adm"
            if type(c_c_adm) is not torch.Tensor:
                c_c_adm = c_c_adm.cond
...
class CFGNoisePredictor:
    ...
    def apply_model(self, *args, **kwargs):
        ...
        if any([getp(p).get("from_smZ", False) for p in cc]) or any(
            [getp(p).get("from_smZ", False) for p in uu]
        ):
        ...
        if (self.is_prompt_editing_c or self.is_prompt_editing_u) and not self.sampler:
            ...
            self.sampler = (
                _find_outer_instance("extra_args", callback=get_sampler) or "unknown"
            )
            ...
            self.steps_multiplier = (
                2
                if any(map(self.sampler.__contains__, second_order_samplers))
                else self.steps_multiplier
            )
            self.steps_multiplier = (
                3
                if any(map(self.sampler.__contains__, third_order_samplers))
                else self.steps_multiplier
            )
        ...
        if self.use_CFGDenoiser is None:
            multi_cc = (
                any(
                    [
                        getp(p)["smZ_opts"].multi_conditioning
                        if "smZ_opts" in getp(p)
                        else False
                        for p in cc
                    ]
                )
            ...
            self.use_CFGDenoiser = getattr(
                _opts, "use_CFGDenoiser", multi_cc or multi_uu
            )
        ...
        if not self.use_CFGDenoiser:
            ...
            if "x" in kwargs:
                kwargs["x"].conds_list = self.inner_model2.conds_list
            else:
                args[0].conds_list = self.inner_model2.conds_list
            if not hasattr(self.inner_model2, "s_min_uncond"):
                self.inner_model2.s_min_uncond = getattr(
                    model_options.get("smZ_opts", None), "s_min_uncond", 0
                )
            if "model_function_wrapper" in model_options:
                model_options["model_function_wrapper_orig"] = model_options.pop(
            ...
            if to_comfy:
                ...
                out = self.inner_model2(
                    ...
                    s_min_uncond=self.inner_model2.s_min_uncond,
...
def sampling_function(
    ...
    cond_pred, uncond_pred = calc_cond_uncond_batch(
        model, cond, uncond_, x, timestep, model_options, cond_scale
    )
    if hasattr(x, "conds_list"):
...
if hasattr(comfy.samplers, "get_area_and_mult"):
    from comfy.samplers import can_concat_cond, cond_cat, get_area_and_mult


def calc_cond_uncond_batch(
    model, cond, uncond, x_in, timestep, model_options, cond_scale_in
):
    conds = []
    ...
    while len(to_run) > 0:
        ...
        if control is not None:
            c["control"] = (
                ...
                else control.get_control(input_x, timestep_, c, len(cond_or_uncond))
            )
    ...
    if a1111:
        ...
        if conds_len != 0:
            ...
            cond_scale = (
                1.0
                / lenc
                * (1.0 if "sampler_cfg_function" in model_options else cond_scale_in)
            )
                ...
                conds_list = [
                    [
                        (
                            ix,
                            1.0
                            if ix > inner_conds_list_len - 1
                            else conds_list[0][ix][1],
                        )
                        for ix in range(conds_len)
                    ]
                ]
            out_cond = out_uncond.clone()
            for cond, (_, weight) in zip(conds, conds_list[0]):
                out_cond += (
                    (cond / (out_count / lenc) - out_uncond) * weight * cond_scale
                )
...
def inject_code(original_func, data):
    ...
    if inspect.ismethod(original_func):
        modified_function = modified_function.__get__(
            original_func.__self__, original_func.__class__
        )
...
def hook_for_settings_node_and_sampling():
    if not hasattr(comfy.samplers, "Sampler"):
        print(
            "[smZNodes]: Your ComfyUI version is outdated. Please update to the latest version."
        )
        comfy.samplers.KSampler.sample = inject_code(
            comfy.samplers.KSampler.sample, payload
        )
    else:
        ...
        def KSampler_sample(*args, **kwargs):
            start_step = get_value_from_args(
                args, kwargs, "start_step", _KSampler_sample
            )
        ...
        def sample(*args, **kwargs):
            ...
            model_options = get_value_from_args(
                args, kwargs, "model_options", _sample, 8
            )
            ...
            if "smZ_opts" in model_options:
                ...
                if hasattr(sampler, "sampler_function"):
                    ...
                    sampler_function_sig_params = inspect.signature(
                        sampler.sampler_function
                    ).parameters
                    params = {
                        x: getattr(opts, x)
                        for x in ["eta", "s_churn", "s_tmin", "s_tmax", "s_noise"]
                        if x in sampler_function_sig_params
                    }
                    sampler.sampler_function = (
                        lambda *a, **kw: sampler.sampler_function_orig(
                            *a, **{**kw, **params}
                        )
                    )
            model.model_options = (
                model_options  # Add model_options to CFGNoisePredictor
            )
...
def hook_for_dtype_unet():
    if hasattr(comfy.model_management, "unet_dtype"):
        ...
        def unet_dtype(device=None, model_params=0, *args, **kwargs):
            dtype = comfy.model_management.unet_dtype_orig(
                device=device, model_params=model_params, *args, **kwargs
            )
            if model_params != 0:
...
def add_sample_dpmpp_2m_alt():
    from comfy.samplers import k_diffusion_sampling
