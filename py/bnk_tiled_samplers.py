...
def slice_gligen(tile_h, tile_h_len, tile_w, tile_w_len, cond, gligen):
    ...
    for emb, h_len, w_len, h, w in gligen_areas:
        ...
        if h < tile_h_end and h_end > tile_h and w < tile_w_end and w_end > tile_w:
            ...
            new_h_end = min(tile_h_end, h_end - tile_h)
            new_w_end = min(tile_w_end, w_end - tile_w)
            gligen_areas_new.append(
                (emb, new_h_end - new_h, new_w_end - new_w, new_h, new_w)
            )
...
def slice_cnet(h, h_len, w, w_len, model: comfy.controlnet.ControlBase, img):
    ...
    model.cond_hint = (
        tiling.get_slice(img, h * 8, h_len * 8, w * 8, w_len * 8)
        .to(model.control_model.dtype)
        .to(get_device())
    )
...
def sample_common(
    ...
    if add_noise == "disable":
        noise = torch.zeros(
            samples.size(), dtype=samples.dtype, layout=samples.layout, device="cpu"
        )
    else:
    ...
    modelPatches, inference_memory = comfy.sampler_helpers.get_additional_models(
        conds, model.model_dtype()
    )
    comfy.model_management.load_models_gpu(
        [model] + modelPatches, model.memory_required(noise.shape) + inference_memory
    )
    ...
    if tiling_strategy != "padded":
        if noise_mask is not None:
            samples += (
                sampler.sigmas[start_at_step].cpu()
                * noise_mask
                * model.model.process_latent_out(noise)
            )
        else:
            samples += sampler.sigmas[
                start_at_step
            ].cpu() * model.model.process_latent_out(noise)

    # cnets
    cnets = [
        c["control"]
        for (_, c) in positive + negative
        if "control" in c and isinstance(c["control"], comfy.controlnet.ControlNet)
    ]
    cnets = list(
        set([x for m in cnets for x in recursion_to_list(m, "previous_controlnet")])
    )
    cnet_imgs = [
        torch.nn.functional.interpolate(
            m.cond_hint_original, (shape[-2] * 8, shape[-1] * 8), mode="nearest-exact"
        ).to("cpu")
        if m.cond_hint_original.shape[-2] != shape[-2] * 8
        or m.cond_hint_original.shape[-1] != shape[-1] * 8
        else None
        for m in cnets
    ]
    ...
    T2Is = [
        c["control"]
        for (_, c) in positive + negative
        if "control" in c and isinstance(c["control"], comfy.controlnet.T2IAdapter)
    ]
    ...
    T2I_imgs = [
        ...
        if m.cond_hint_original.shape[-2] != shape[-2] * 8
        or m.cond_hint_original.shape[-1] != shape[-1] * 8
        or (m.channels_in == 1 and m.cond_hint_original.shape[1] != 1)
        else None
        for m in T2Is
    ]
    T2I_imgs = [
        torch.mean(img, 1, keepdim=True)
        if img is not None and m.channels_in == 1 and m.cond_hint_original.shape[1]
        else img
    ...
    spatial_conds_pos = [
        (
            ...
            comfy.sample.prepare_mask(c[1]["mask"], shape, device)
            if "mask" in c[1]
            else None,
        )
        for c in positive
    ...
    # gligen
    gligen_pos = [c[1]["gligen"] if "gligen" in c[1] else None for c in positive]
    gligen_neg = [c[1]["gligen"] if "gligen" in c[1] else None for c in negative]

    gen = torch.manual_seed(noise_seed)
    if tiling_strategy == "random" or tiling_strategy == "random strict":
        tiles = tiling.get_tiles_and_masks_rgrid(
            end_at_step - start_at_step, samples.shape, tile_height, tile_width, gen
        )
    elif tiling_strategy == "padded":
        tiles = tiling.get_tiles_and_masks_padded(
            end_at_step - start_at_step, samples.shape, tile_height, tile_width
        )
    else:
        tiles = tiling.get_tiles_and_masks_simple(
            end_at_step - start_at_step, samples.shape, tile_height, tile_width
        )
    ...
    total_steps = sum(
        [
            ...
            for steps_list in img_pass
            for _, _, _, _, num_steps, _ in steps_list
        ]
    )
    current_step = [0]
        ...
        for img_pass in tiles:
            for i in range(len(img_pass)):
                for (
                    ...
                    tiled_mask = None
                    if noise_mask is not None:
                        tiled_mask = tiling.get_slice(
                            noise_mask, tile_h, tile_h_len, tile_w, tile_w_len
                        ).to(device)
                    ...
                    if (
                        ...
                        or tiling_strategy == "random strict"
                    ):
                        tile_h, tile_h_len, tile_w, tile_w_len, tiled_mask = (
                            tiling.mask_at_boundary(
                                tile_h,
                                tile_h_len,
                                tile_w,
                                tile_w_len,
                    ...
                    tiled_latent = tiling.get_slice(
                        samples, tile_h, tile_h_len, tile_w, tile_w_len
                    ).to(device)

                    if tiling_strategy == "padded":
                        tiled_noise = tiling.get_slice(
                            noise, tile_h, tile_h_len, tile_w, tile_w_len
                        ).to(device)
                    else:
                    ...
                    pos = [
                        slice_cond(
                            tile_h, tile_h_len, tile_w, tile_w_len, c, area, device
                        )
                    ...
                    for cond, gligen in zip(pos, gligen_pos):
                        slice_gligen(
                            tile_h, tile_h_len, tile_w, tile_w_len, cond, gligen
                        )
                    ...
                    tile_result = sampler.sample(
                        tiled_noise,
                        pos,
                        neg,
                        cfg=cfg,
                        latent_image=tiled_latent,
                        start_step=start_at_step + i * tile_steps,
                        last_step=start_at_step + i * tile_steps + tile_steps,
                        force_full_denoise=force_full_denoise
                        and i + 1 == end_at_step - start_at_step,
                        denoise_mask=tiled_mask,
                        callback=callback,
                        disable_pbar=True,
                        seed=noise_seed,
                    )
