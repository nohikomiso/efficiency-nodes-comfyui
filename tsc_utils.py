...
def quick_resize(source_tensor: torch.Tensor, target_shape: tuple) -> torch.Tensor:
    ...
    for img in source_tensor:
        resized_pil = tensor2pil(img.squeeze(0)).resize(
            (target_shape[2], target_shape[1]), Image.ANTIALIAS
        )
...
def extract_node_info(prompt, id, indirect_key=None):
    ...
    if indirect_key:
        ...
        if (
            id in prompt
            and "inputs" in prompt[id]
            and indirect_key in prompt[id]["inputs"]
        ):
...
def print_loaded_objects_entries(id=None, prompt=None, show_id=False):
    ...
    if prompt is not None and id is not None:
        ...
        print("\033[36mGlobal Models Cache:\033[0m")
    else:
    ...
    for key in ["ckpt", "refn", "vae", "lora"]:
        entries_with_id = (
            loaded_objects[key]
            if id is None
            else [entry for entry in loaded_objects[key] if id in entry[-1]]
        )
        if (
            not entries_with_id
        ):  # If no entries with the chosen ID, print None and skip this key
            continue
        entries_found = True
        print(f"{key.capitalize()}:")
        for i, entry in enumerate(entries_with_id, 1):  # Start numbering from 1
            if key == "lora":
                base_ckpt_name = os.path.splitext(os.path.basename(entry[1]))[
                    0
                ]  # Split logic for base_ckpt
                if id is None:
                    associated_ids = ", ".join(
                        map(str, entry[-1])
                    )  # Gather all associated ids
                    print(
                        f"  [{i}] base_ckpt: {base_ckpt_name} (ids: {associated_ids})"
                    )
                else:
...
def load_checkpoint(
    ckpt_name, id, output_vae=True, cache=None, cache_overwrite=True, ckpt_type="ckpt"
):
    global loaded_objects
    ...
    for entry in loaded_objects[ckpt_type]:
        if entry[0] == ckpt_name:
            ...
            cache_full = (
                cache
                and len(
                    [entry for entry in loaded_objects[ckpt_type] if id in entry[-1]]
                )
                >= cache
            )
    ...
    clip = out[1]
    vae = (
        out[2] if output_vae else None
    )  # Load VAE from the checkpoint path only if output_vae is True
        ...
        if entry[0] == vae_name:
            ...
            if id not in ids:
                if (
                    cache
                    and len(
                        [entry for entry in loaded_objects["vae"] if id in entry[-1]]
                    )
                    >= cache
                ):
                    return vae
...
def load_lora(
    lora_params, ckpt_name, id, cache=None, ckpt_cache=None, cache_overwrite=False
):
    global loaded_objects
    ...
    for entry in loaded_objects["lora"]:
        ...
        if set(entry[0]) == set(lora_params) and entry[1] == ckpt_name:
            ...
            cache_full = (
                cache
                and len([entry for entry in loaded_objects["lora"] if id in entry[-1]])
                >= cache
            )
    ...
    def recursive_load_lora(
        lora_params, ckpt, clip, id, ckpt_cache, cache_overwrite, folder_paths
    ):
    ...
    lora_model, lora_clip = recursive_load_lora(
        lora_params, ckpt, clip, id, ckpt_cache, cache_overwrite, folder_paths
    )

    if cache:
        if len([entry for entry in loaded_objects["lora"] if id in entry[-1]]) < cache:
            loaded_objects["lora"].append(
                (lora_params, ckpt_name, lora_model, lora_clip, [id])
            )
        else:
            clear_cache(id, cache, "lora")
            if cache_overwrite:
...
def clear_cache(id, cache, dict_name):
    ...
    id_associated_entries = [
        entry for entry in loaded_objects[dict_name] if id in entry[-1]
    ]
...
def clear_cache_by_exception(
    node_id, vae_dict=None, ckpt_dict=None, lora_dict=None, refn_dict=None
):
    global loaded_objects
        ...
        for tuple_idx, tuple_item in enumerate(loaded_objects[dict_name].copy()):
            if arg_name == "lora_dict":
                ...
                for lora_params, ckpt_name in arg_val:
                    ...
                    if (
                        set(lora_params) == set(tuple_item[0])
                        and ckpt_name == tuple_item[1]
                    ):
                        break
                ...
                tuple_item[0] not in arg_val
            ):  # Only remove the tuple if it's not in arg_val
                if node_id in tuple_item[-1]:
                    tuple_item[-1].remove(node_id)
                    if not tuple_item[-1]:
                        loaded_objects[dict_name].remove(tuple_item)
...
def print_last_helds(id=None):
    ...
    if id is not None:
        ...
        print("Global Last Helds:")
    for key in ["preview_images", "latent", "output_images", "vae_decode"]:
        entries_with_id = (
            last_helds[key]
            if id is None
            else [entry for entry in last_helds[key] if id == entry[-1]]
        )
        if (
            not entries_with_id
        ):  # If no entries with the chosen ID, print None and skip this key
...
destination_dir = os.path.join(
    comfy_dir, "web", "extensions", "efficiency-nodes-comfyui"
)
